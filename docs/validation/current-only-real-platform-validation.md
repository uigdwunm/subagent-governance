...
日期：2026-08-24  
结论：`partial_validation_failed_at_v1`（安装前启动的 Codex 宿主仍存活，真实 PreToolUse 引用已不存在的 `rc.14` Hook 路径；没有热修、重装或绕过 Hook）
...
V1 使用已加载的 `$subagent-governance` Skill 进行了一个安全的 unmanaged 原生 `spawn_agent`：task name 为 `p10b_unmanaged_probe`（无 `sg_` 治理前缀），`fork_turns=none`，模型为 `gpt-5.6-terra` / `high`。子任务仅被允许返回固定标记 `P10B_UNMANAGED_PROBE_OK`；禁止调用工具、访问文件、读写、派发或通信。该调用未创建 native target、tool-use 或治理状态。平台在 PreToolUse 直接拒绝，实际工具响应为：
...
- 这发生在安装后新建的独立任务，是实际原生工具和 Hook 交互，而不是 fixture、手工运行 router 或源码推断。
- cache 父目录只含目标 `rc.13` version；不存在报错中的 `rc.14` cache。事后只读进程检查证明 Codex/ChatGPT 桌面主进程与 `codex-code-mode-host` 都从安装前持续存活，所谓“完全退出并重新打开”未实际发生。因此证据只支持旧宿主持有旧 Hook command/path，不支持“完整宿主重启后仍缓存旧路径”的结论。
...
| 场景 | 状态 | 实际证据与边界 |
| --- | --- | --- |
| V1 插件/Hook 基线、unmanaged 放行 | `failed` | `codex plugin list` 和安装检查确认目标文件版本/单 cache/digest 健康；但安装前启动的宿主仍在运行，真实 unmanaged `spawn_agent` 在 PreToolUse 被不存在的 `rc.14` 路径阻断。无 target/tool-use/state；unknown/unmatched 零写入和成功 fail-open 均未取得。 |
...
本次不是 P10 完成，也不是 release-ready 结论。用户需要先完全退出并重新打开 Codex/ChatGPT App，且只读确认桌面主进程与 `codex-code-mode-host` 都已重建；不得直接编辑 trust/cache。随后必须新建另一个独立 P10-B 任务，从 V1 重新验证 V1–V7。如果完整宿主重启后仍引用旧路径，才进入新的开发诊断/修复与重新部署流程。
