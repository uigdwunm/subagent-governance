...
- 日期：2026-08-25
- 适用范围：下一轮架构收口与验收
- 实施状态：本地实现完成；state-v9 / TaskContract v2、最小 lifecycle、allowlisted runtime bundle 与单一开发部署入口均已落地；未经授权的安装与真实平台验证仍未执行，当前运行边界以 `docs/architecture.md` 为准
...
2. 只读 SessionStart：始终注入 Hook stdin 的当前权威 exact session ID 与当前已安装 CLI entrypoint，并 best-effort 展示该 Session 的未关闭 task 摘要和派生下一步。
...
SessionStart 不得创建目录、lock 或空状态，不 cleanup、migrate、rebuild、自动关闭、自动重试、扫描其他 Session 或调用原生工具。治理命令必须使用其注入的已安装 CLI entrypoint，不能用工作区相对脚本、其他 cache 版本或猜测路径替代；否则 prepare 与 Hook 可能落到不同数据根。`<codex_delegation><source_thread_id>` 只表示来源任务，不能替代当前 Hook 的 session ID。SessionStart 未投递不影响 ledger correctness，但必须停止新的 governed dispatch；显式只读 status 命令只为已有 exact identity 和 CLI authority 提供状态兜底。
