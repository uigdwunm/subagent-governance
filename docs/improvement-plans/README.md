...
> 当前方向已由 [减法收口目标架构](../architecture-reduction-adr.md) 和
> [reduction cutover](reduction-cutover.md) 取代。P1–P14 保留为已经实施或验证过的
> 历史改造记录，不再作为下一轮实施顺序。其中 P12-A 仅作为已完成实验归档，
> P12-B 在当前平台证据和目标架构下为 rejected/archived，不得继续实施。

本目录的 P1–P14 记录 2026-08-24 审查后确认并执行的 current-only 历史改造方案；架构基线使用上方 reduction cutover，后续专项方案见下表。历史执行约束不自动适用于专项方案。
