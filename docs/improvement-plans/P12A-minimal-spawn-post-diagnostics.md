...
P12-A probe 是临时真实验证设施，不是长期第二套事件或诊断架构，也不得直接进入稳定发布：

- 若真实证据激活 P12-B，P12-B 必须把仍有价值的最小字段吸收到最终 receipt/index 设计，并在同一实施序列删除 `spawn-post-probe-ids-v1`、`spawn-post-probes-v1` 及 P12-A 专用路由；不得让 probe 与最终机制并存。
- 若真实证据要求停止 P12-B，应提交一个只移除 P12-A runtime probe 的清理改动，保留验证报告和平台边界说明；随后恢复不含 probe 的测试安装。
