"""Strictly read-only diagnostics for current v8 state files.
...
def _read_session_file_read_only(path: Path, *, requested_session: str | None = None) -> dict[str, Any]:
    ...
    if value.get("state_format_version") != STATE_FORMAT_VERSION:
        raise DiagnosticReadError("unsupported_format", "仅支持当前 state-v8 格式；历史状态不会被解释或迁移", context={"path": str(path), "session_id": stored})
    violations = validate_current_state_format(value)
    if violations:
        paths = ", ".join(str(issue.path) for issue in violations[:8])
        raise DiagnosticReadError("current_format_invalid", "当前 v8 状态格式非法：" + paths, context={"path": str(path), "session_id": stored})
    return value
