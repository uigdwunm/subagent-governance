...
运行时还会观察 `list_agents` 的明确 `errored` 状态，将 provider 流故障记录为 `platform_error`，避免任务长期停留在假 `running`。同一任务允许一次自动恢复；恢复后平台错误再次出现时转为 `needs_decision`，防止无收益的无限重试。插件只能诊断和辅助恢复，不能修复 provider 的流传输。
