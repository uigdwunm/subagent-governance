...
- Prepared expiry prevents a new claim; it does not delete the record. Open records are not automatically cleared. Ledger writes retain at most the newest 64 closed records. When a new task would exceed the 512-task limit or the 3 MiB admission limit, the oldest whole closed records are also evicted in ascending order of close time, creation time, and task ID; prepare reports their IDs in `pruned_task_ids`. Eviction and insertion share one transaction; rejected admission leaves the original ledger unchanged. There is no timed deletion service. State-v12 does not read, migrate, or delete older ledgers.
