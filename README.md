...
StateStore 只接受严格的 `state_format_version=8` 与 `state-v8` 命名空间。根记录、task、execution、pending、health、tombstone、agent 和 group 都使用关闭字段集合；缺少版本、版本不匹配、`managed=false` 或任何未知持久化字段都会直接拒绝，且不读取、迁移、删除旧 `state-v1`、`state-v6` 或 `state-v7`。
