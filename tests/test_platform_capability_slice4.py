...
FIXTURE = ROOT / "tests/fixtures/state-v1-four-plane-migration.json"
SPEC = importlib.util.spec_from_file_location("subagent_governance_slice_4", SCRIPT)
...
class PlatformCapabilitySlice4Tests(unittest.TestCase):
    def setUp(self):
        self.temporary = tempfile.TemporaryDirectory()
        self.store = governance.StateStore(Path(self.temporary.name))
        self.state = governance._migrate_state_to_current(
            json.loads(FIXTURE.read_text(encoding="utf-8"))
        )
        self.session_id = str(self.state["session_id"])
        self.task_id = "sg-slice-2-legacy"
        self.target = self.state["tasks"][self.task_id]["executions"]["1"][
            "dispatch_record"
        ]["dispatch_target"]
        ...
        return governance.handle(
            {
                ...
                "tool_use_id": f"slice-4-list-{now}",
    ...
    def test_slice4_machine_contract_freezes_adapter_freshness_and_stop(self):
        ...
        self.assertEqual(
            governance.LIST_AGENTS_WRAPPER_STATUS_PARSE_POLICY,
            adapter["wrapper_status_parse_policy"],
        )
        self.assertEqual(
            governance.LIST_AGENTS_MALFORMED_WRAPPER_POLICY,
        ...
        self.assertEqual(capability["positive_parent_stop_gate"], "none_through_slice_4")
    ...
    def test_nested_or_summary_agents_are_not_scanned(self):
        ...
        for index, response in enumerate(
            (
                ...
                {"content": [{"type": "text", "text": json.dumps(nested_agent)}]},
    ...
    def test_malformed_or_explicit_error_wrappers_do_not_expose_agents(self):
        ...
        responses = (
            ...
            json.dumps({"error": {"message": "boom"}, **completed_agent}),
        )
