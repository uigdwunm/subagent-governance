...
class P7ViewsGroupsSessionsDiagnosticsTests(unittest.TestCase):
    ...
    def add_task(self, task_id="task-a"):
        ...
        record = governance._initial_task_record(1, "0123456789ab", "sg_standard_p7_t_0123456789ab", contract, 100)
    ...
    def test_group_is_strict_and_trims_canonical_identifiers(self):
        ...
        result = governance.upsert_group({"group_id": " group ", "objective_summary": " objective ", "members": [{"task_id": f" {task_id} ", "required": True}]}, self.session_id, state_store=self.store)
        ...
        with self.assertRaises(governance.GroupValidationError):
            governance.upsert_group({"group_id": "x", "objective_summary": "x", "members": [], "extra": True}, self.session_id, state_store=self.store)
        class MissingGroupsStore:
            def update(self, _session_id, callback, **_kwargs):
                return callback({"tasks": {task_id: {}}, "agents": {}})
        with self.assertRaises(governance.GroupValidationError):
            governance.upsert_group({"group_id": "missing-root", "objective_summary": "x", "members": []}, self.session_id, state_store=MissingGroupsStore())
    ...
    def test_session_end_keeps_degraded_health_even_when_work_item_tombstoned(self):
        ...
        governance._close_attempt_record(state, task_id, 1, execution, "parent_closed", 120)
    ...
    def test_session_start_keeps_readable_summary_after_one_maintenance_failure(self):
        ...
        with mock.patch.object(self.store, "cleanup_expired_tombstones", side_effect=RuntimeError("cleanup failed")):
            result = governance.handle({"session_id": self.session_id, "hook_event_name": "SessionStart"}, self.store)
    ...
    def test_diagnostics_v5_is_unsupported_and_zero_write(self):
        ...
        document, exit_code = governance._build_diagnostic_document(self.session_id, self.root)
